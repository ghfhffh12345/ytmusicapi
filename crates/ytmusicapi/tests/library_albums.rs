...
#[tokio::test]
async fn get_library_albums_parses_audit_first_page() {
    let (_server, _dir, client) = audit_client_with_browse_response(include_str!(
        "fixtures/audit/raw/library/albums/first_page.json"
    ))
    .await;

    let albums = client.get_library_albums().await.unwrap();

    assert_eq!(albums.items.len(), 25);
    assert_eq!(
        albums.continuation,
        Some(LibraryAlbumsContinuationToken::new("REDACTED_TOKEN"))
    );
}

#[tokio::test]
async fn get_library_albums_audit_continuation_is_terminal() {
    let (_server, _dir, client) = audit_client_with_browse_response(include_str!(
        "fixtures/audit/raw/library/albums/continuation.json"
    ))
    .await;

    let albums = client
        .get_library_albums_continuation(LibraryAlbumsContinuationToken::new("album-token-1"))
        .await
        .unwrap();

    assert_eq!(albums.items.len(), 29);
    assert_eq!(albums.continuation, None);
}

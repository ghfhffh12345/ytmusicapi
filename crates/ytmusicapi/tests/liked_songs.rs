...
#[tokio::test]
async fn get_liked_songs_parses_audit_split_first_page() {
    let (_server, _dir, client) = audit_client_with_browse_response(include_str!(
        "fixtures/audit/raw/library/liked_songs/first_page.json"
    ))
    .await;

    let liked_songs = client.get_liked_songs().await.unwrap();

    assert_eq!(liked_songs.title, "REDACTED_TEXT");
    assert_eq!(liked_songs.items.len(), 100);
    assert_eq!(
        liked_songs.continuation,
        Some(LikedSongsContinuationToken::new("REDACTED_TOKEN"))
    );
}
